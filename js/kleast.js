...
function drawCareer() {
    ...
    const select_statement = 'SELECT * WHERE A = "Tam Man Ho Jeremy Jansen" OR A = "Tse Wai Chun Paul" OR A = "Wong Kwok Kin" OR A = "Wu Chi Wai"';
...
function drawGoogleTrends() {
    let query_statement = data_source + 'sheet=trends&headers=1&tq=';
    query_statement += encodeURIComponent('SELECT A, J, K, L, M');
...
function drawInvestigate() {
    ...
    query_statement += encodeURIComponent('SELECT * WHERE A = "Tam Man Ho Jeremy Jansen" OR A = "Tse Wai Chun Paul" OR A = "Wong Kwok Kin" OR A = "Wu Chi Wai"');
