...
function drawCareer() {
    ...
    const select_statement = 'SELECT * WHERE A = "Cheng Chung Tai" OR A = "Chu Hoi Dick Eddie" OR A = "Leung Che Cheung" OR A = "Tien Michael Puk Sun"';
...
function drawGoogleTrends() {
    let query_statement = data_source + 'sheet=trends&headers=1&tq=';
    query_statement += encodeURIComponent('SELECT A, N, O, P, Q');
...
function drawInvestigate() {
    ...
    query_statement += encodeURIComponent('SELECT * WHERE A = "Cheng Chung Tai" OR A = "Chu Hoi Dick Eddie" OR A = "Leung Che Cheung" OR A = "Tien Michael Puk Sun"');
